...
void main() {
  group('AppState plan progression', () {
    test('does not expire active plan when no queued plan exists', () async {
      ...
      // Trigger progression through markAttendance with remaining=0
      await state.markAttendance(1, 'present', '1404/06/21');

      final active = state.activePlanForClient(1);
      expect(active?.status, 'active');
    });

    test('expires active plan and promotes queued plan when remaining is 0', () async {
      final state = createTestAppState();
      ...
      final active = state.activePlanForClient(1);
      expect(active?.status, 'active');
      expect(active?.remaining, 4);
      expect(active?.startDate, isNotNull);
    });
  });
}
